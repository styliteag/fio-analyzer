...
async def lifespan(app: FastAPI):
    ...
    # Startup
    log_info(
        "Starting FIO Analyzer FastAPI Backend",
        {
            "version": "1.0.0",
    ...
    # Shutdown
    log_info("Shutting down FIO Analyzer FastAPI Backend", {"process_id": os.getpid()})
...
For detailed examples and testing, visit the interactive documentation below.
    ...
    servers=[
        ...
        {"url": "http://example.intern", "description": "Production server"},
    ],
    tags_metadata=[
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    log_error(
        "Unhandled exception",
        exc,
        {"request_id": request_id, "method": request.method, "url": str(request.url)},
    )
...
def signal_handler(signum, frame):
    ...
    log_info("Received shutdown signal", {"signal": signum, "process_id": os.getpid()})
