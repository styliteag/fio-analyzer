...
class TestRunBase(BaseModel):
    ...
    read_write_pattern: str = Field(
        ..., 
        description="I/O access pattern",
        example="randread",
        max_length=50
    )
    ...
    fio_version: Optional[str] = Field(
        None,
        description="FIO version used for testing",
        example="fio-3.33"
    )
    ...
    usr_cpu: Optional[float] = Field(
        None,
        description="User CPU utilization percentage",
        example=15.5,
        ge=0,
        le=100
    )
    ...
    output_file: Optional[str] = Field(
        None,
        description="FIO output filename",
        example="testfile"
    )
    ...
    sync: Optional[int] = Field(
        None,
        description="Sync flag (0=async, 1=sync)",
        example=0,
        ge=0,
        le=1
    )
    iodepth: Optional[int] = Field(
        None,
        description="I/O depth (same as queue_depth)",
        example=32,
        ge=1
    )
...
class ServerInfo(BaseModel):
    ...
    config_count: int = Field(
        ...,
        description="Number of unique test configurations",
        example=15,
        ge=0
    )
    ...
    last_test_time: str = Field(
        ...,
        description="Most recent test timestamp",
        example="2025-06-31T20:00:00"
    )
    first_test_time: str = Field(
        ...,
        description="Oldest test timestamp",
...
class TrendDataPoint(BaseModel):
    ...
    block_size: str = Field(
        ...,
        description="Block size for this data point",
        example="4K"
    )
    read_write_pattern: str = Field(
        ...,
        description="I/O pattern for this data point",
        example="randread"
    )
    queue_depth: int = Field(
        ...,
        description="Queue depth for this data point",
        example=32
    )
    value: float = Field(
        ...,
    ...
    moving_avg: Optional[float] = Field(
        None,
        description="3-point moving average",
        example=123500.0
    )
...
class TrendAnalysis(BaseModel):
    ...
    first_value: float = Field(
        ...,
        description="First chronological value",
        example=118000.0
    )
    last_value: float = Field(
        ...,
        description="Last chronological value",
        example=122000.0
    )
    overall_change: str = Field(
        ...,
        description="Overall percentage change from first to last",
        example="+3.4%"
    )
...
class FilterOptions(BaseModel):
    ...
    syncs: List[int] = Field(
        ...,
        description="Available sync flag values",
        example=[0, 1]
    )
    queue_depths: List[int] = Field(
        ...,
        description="Available queue depths",
        example=[1, 8, 16, 32, 64]
    )
    directs: List[int] = Field(
        ...,
        description="Available direct I/O flag values",
        example=[0, 1]
    )
    num_jobs: List[int] = Field(
        ...,
        description="Available job count values",
        example=[1, 4, 8, 16]
    )
    test_sizes: List[str] = Field(
        ...,
        description="Available test sizes",
        example=["1G", "10G", "100G"]
    )
...
class ImportResponse(BaseModel):
    ...
    test_run_id: int = Field(
        ...,
        description="ID of the newly created test run",
        example=42
    )
    filename: str = Field(
        ...,
...
class BulkImportStatistics(BaseModel):
    ...
    totalTestRuns: int = Field(
        ...,
        description="Number of test runs imported",
        example=25,
        ge=0
    )
...
class BulkImportResponse(BaseModel):
    ...
    dryRunResults: Optional[List[BulkImportDryRunResult]] = Field(
        None,
        description="Dry run results (only present when dryRun=true)"
    )
...
class BulkDeleteResponse(BaseModel):
    ...
    deleted: int = Field(
        ...,
        description="Number of successfully deleted records",
        example=8,
        ge=0
    )
    notFound: int = Field(
        ...,
        description="Number of records that were not found",
        example=2,
        ge=0
    )
...
class APIInfo(BaseModel):
    ...
    documentation: str = Field(
        ...,
        description="Primary documentation URL",
        example="/docs"
    )
