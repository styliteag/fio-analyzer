...
def get_current_user(request: Request) -> Optional[User]:
    ...
    log_debug("Auth check - all headers", {
        "request_id": request_id,
        "headers": dict(request.headers)
    })
    
    auth_header = request.headers.get("authorization")
    log_debug("Auth check - authorization header", {
        "request_id": request_id,
    ...
    if not credentials:
        log_debug("Auth check - failed to parse auth header", {
            "request_id": request_id,
    ...
    log_debug("Auth check - parsed credentials", {
        "request_id": request_id,
        "username": username,
    ...
    log_debug("Auth check - role lookup result", {
        "request_id": request_id,
        "username": username,
        "role": role
    })
    ...
    if role:
        log_debug("Auth check - user authenticated successfully", {
            "request_id": request_id,
            "username": username,
            "role": role
        })
    ...
    log_debug("Auth check - authentication failed", {
        "request_id": request_id,
        "username": username,
...
def require_auth(request: Request) -> User:
    ...
    if not user:
        log_debug("Authentication denied - no valid credentials", {
            "request_id": getattr(request.state, 'request_id', 'unknown'),
...
def require_admin(request: Request) -> User:
    ...
    if not user:
        log_debug("Admin access denied - no auth header", {
            "request_id": getattr(request.state, 'request_id', 'unknown'),
            "ip": request.client.host if request.client else "unknown",
            "user_agent": request.headers.get("user-agent", "unknown")
        })
...
def require_uploader(request: Request) -> User:
    ...
    if not user:
        log_debug("Upload access denied - no auth header", {
            "request_id": getattr(request.state, 'request_id', 'unknown'),
            "ip": request.client.host if request.client else "unknown",
            "user_agent": request.headers.get("user-agent", "unknown")
        })
