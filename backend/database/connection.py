...
class DatabaseManager:
    ...
    async def connect(self):
        ...
        try:
            ...
            log_info("Connected to SQLite database successfully", {"db_path": str(self.db_path)})
    ...
    async def _populate_sample_data(self, cursor: sqlite3.Cursor):
        ...
        servers = [
            {"hostname": "sim-web-01", "protocol": "NFS", "drives": [("Samsung 980 PRO", "HDD")]},
            {"hostname": "sim-db-01", "protocol": "iSCSI", "drives": [("Samsung SN850", "NVMe SSD")]},
            {"hostname": "sim-app-01", "protocol": "Local", "drives": [("Samsung 980 PRO", "NVMe SSD")]},
            {"hostname": "sim-cache-01", "protocol": "Local", "drives": [("Samsung Optane", "NVMe SSD")]},
        ]
    ...
    async def update_latest_flags(self, test_run_data: Dict[str, Any]):
        ...
        log_info("Updating latest flags for test run", {
        ...
        self.connection.commit()
        log_info(f"Updated {cursor.rowcount} existing tests to is_latest=0", {"unique_key": unique_key})
