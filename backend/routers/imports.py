...
async def bulk_import_fio_data(
    ...
    bulk_request: Dict[str, Any] = Body(
        ...
        example={"overwrite": False, "dryRun": False},
    ),
            ...
            try:
                ...
                if info_file.exists():
                    try:
                        ...
                        log_error(
                            f"Error reading .info file {info_file}",
                            e,
                            {"request_id": request_id},
                        )
...
def insert_test_run(db: sqlite3.Connection, test_run_data: Dict[str, Any], file_path: str = None) -> int:
    ...
    if file_path:
        cursor.execute(
            "UPDATE test_runs SET uploaded_file_path = ? WHERE id = ?",
            (str(file_path), test_run_id),
        )
        cursor.execute(
            "UPDATE test_runs_all SET uploaded_file_path = ? WHERE id = ?",
...
def create_metadata_file(file_path: str, test_run_data: Dict[str, Any], username: str, original_filename: str) -> str:
    ...
    log_info(
        ...
        {"metadata_path": str(metadata_path), "metadata_keys": list(metadata.keys())},
    )
