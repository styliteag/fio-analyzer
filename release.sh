...
update_changelog() {
    ...
    echo "" >> "$temp_changelog"
    echo "All notable changes to FIO Analyzer will be documented in this file." >> "$temp_changelog"
    echo "" >> "$temp_changelog"
...
main() {
    ...
    print_info "  • styliteag/fio-analyzer  :$new_version"
    print_info "  • ghcr.io/$(git config --get remote.origin.url | sed 's/.*github.com[/:]//g' | sed 's/.git$//')/fio-analyzer:$new_version"
    print_info ""
