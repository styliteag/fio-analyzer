...
// Component for individual data bars in 3D space
const DataBar: React.FC<{
    point: PerformancePoint;
    position: [number, number, number];
    height: number;
    onHover: (point: PerformancePoint | null) => void;
}> = ({ point, position, height, onHover }) => {
    ...
    // Position adjustment: The box geometry is centered at (0,0,0), so we need to shift it up by height/2
    // to make it look like it's standing on the floor (y=0).
    // The 'position' prop passed in has y as the value on the axis, but for the bar, 
    // we want the base at y=0 and top at y=height.
    // So the center y should be height / 2.
    const [x, , z] = position;
    ...
    return (
        <mesh
            ...
            onPointerOver={(e) => {
                e.stopPropagation(); // Prevent hovering multiple bars if they overlap in view
                setHovered(true);
                onHover(point);
            }}
            onPointerOut={() => {
        ...
        >
            ...
            />
            {/* Optional: Add a wireframe or edge highlight for better visibility */}
            {hovered && (
                <lineSegments>
                ...
                {[0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5].map(i => (
                    <React.Fragment key={`floor-${i}`}>
                        ...
                        <mesh position={[2.5, 0.001, i]} rotation={[-Math.PI / 2, 0, Math.PI / 2]}>
                            ...
                            <meshBasicMaterial color={colorScheme.floorGrid} transparent opacity={0.3} />
                        </mesh>
                        {/* Grid lines along Z */}
                ...
                {[0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5].map(i => (
                    <React.Fragment key={`back-wall-${i}`}>
                        ...
                        <mesh position={[2.5, i, 0.001]}>
                            <planeGeometry args={[5, 0.02]} />
                            <meshBasicMaterial color={colorScheme.wallGrid} transparent opacity={0.2} />
                        </mesh>
                        {/* Vertical lines */}
            ...
            {points.map((point, index) => {
                ...
                return (
                    <DataBar
                        key={index}
                        point={point}
                        position={[x, y, z]} // y passed here is the target height (scaled value)
                        height={y} // The height of the bar is the scaled Y value
...
const Performance3DChart: React.FC<Performance3DChartProps> = ({ drives, allDrives }) => {
    ...
    const [fov, setFov] = React.useState<number>(35);
    const [cameraPosition, setCameraPosition] = React.useState<[number, number, number]>([10, 10, 10]);
        ...
        <div className={`w-full ${isMaximized ? 'fixed inset-0 z-50 bg-white dark:bg-gray-900 p-8 overflow-y-auto' : ''}`}>
            <div className="mb-4">
                <div className="flex items-center justify-between mb-2">
                    <h4 className="text-lg font-semibold theme-text-primary">
                        3D Performance Bar Chart {isMaximized && '- Maximized'}
                    </h4>
                    <button
                        onClick={() => setIsMaximized(!isMaximized)}
                        className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors"
                        title={isMaximized ? "Exit fullscreen" : "Maximize visualization"}
                    >
                        {isMaximized ? (
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                    ...
                    {Object.entries(colorSchemes).map(([key, scheme]) => (
                        <button
                            ...
                            className={`px-3 py-1 rounded-md text-xs font-medium transition-all ${colorScheme === key
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                    ...
                    <button
                        ...
                        className={`px-3 py-1 rounded-md text-xs font-medium transition-all ${cameraMode === 'perspective'
                    ...
                    <button
                        ...
                        className={`px-3 py-1 rounded-md text-xs font-medium transition-all ${cameraMode === 'orthographic'
                            ? 'bg-green-500 text-white shadow-md'
                            : 'bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 theme-text-secondary'
                            }`}
                    >
            ...
            <div className="flex flex-col lg:flex-row gap-6">
                ...
                <div className="flex-1">
                    <div className={`relative bg-gray-50 dark:bg-gray-900 rounded-lg p-4 ${isMaximized ? 'h-[80vh]' : 'h-[600px]'}`}>
                        <Canvas
                                ...
                                {drives.map((drive, index) => (
                                    <div key={`${drive.hostname || 'unknown'}-${drive.protocol}-${drive.drive_type}-${drive.drive_model}-${index}`} className="flex items-center gap-2 text-xs">
                                        <div
                                            ...
                                            style={{ backgroundColor: driveColors[index] || currentScheme.drives[index % currentScheme.drives.length] }}
                                        ></div>
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-lg p-4 border">
                        ...
                        <div className="space-y-4">
                            {drives.map((drive, index) => {
                                ...
                                const avgIOPS = drivePoints.reduce((sum, p) => sum + p.y, 0) / drivePoints.length;
                                const avgLatency = drivePoints.reduce((sum, p) => sum + p.x, 0) / drivePoints.length;
                                ...
                                return (
                                    <div key={`${drive.hostname || 'unknown'}-${drive.protocol}-${drive.drive_type}-${drive.drive_model}-summary-${index}`} className="border-l-4 pl-3" style={{ borderColor: driveColors[index] || currentScheme.drives[index % currentScheme.drives.length] }}>
