...
const isValidFilterOptions = (obj) => {
    return (typeof obj === 'object' &&
...
describe('API Contract Tests', () => {
    beforeAll(async () => {
        ...
        try {
            const health = await apiClient.get(`${API_BASE_URL}/health`);
    ...
    describe('Test Runs Endpoints', () => {
        it('GET /api/test-runs/ should return array of TestRun objects', async () => {
            const response = await apiClient.get(`${API_BASE_URL}/api/test-runs/`);
        ...
        it('GET /api/test-runs/ should handle filter parameters', async () => {
            ...
            const filters = await apiClient.get(`${API_BASE_URL}/api/filters/`);
            if (filters.hostnames.length > 0) {
                ...
                const response = await apiClient.get(`${API_BASE_URL}/api/test-runs/?${params.toString()}`);
                expect(Array.isArray(response)).toBe(true);
        ...
        it('GET /api/test-runs/{id} should return single TestRun', async () => {
            ...
            const testRuns = await apiClient.get(`${API_BASE_URL}/api/test-runs/?limit=1`);
            if (testRuns.length === 0) {
                console.warn('No test runs available for single test run contract test');
                return;
            }
            const testRun = await apiClient.get(`${API_BASE_URL}/api/test-runs/${testRuns[0].id}`);
    ...
    describe('Filter Endpoints', () => {
        it('GET /api/filters should return FilterOptions', async () => {
            const response = await apiClient.get(`${API_BASE_URL}/api/filters/`);
    ...
    describe('User Management Endpoints (Admin Only)', () => {
        it('GET /api/users should return array of UserAccount objects', async () => {
            const response = await apiClient.get(`${API_BASE_URL}/api/users/`);
            expect(Array.isArray(response)).toBe(true);
            if (response.length > 0) {
        ...
        it('POST /api/users should create and delete user', async () => {
            ...
            try {
                // Create user
                const createResponse = await apiClient.post(`${API_BASE_URL}/api/users/`, newUser);
                ...
                // Verify user exists
                const users = await apiClient.get(`${API_BASE_URL}/api/users/`);
                ...
                try {
                    const deleteResponse = await apiClient.delete(`${API_BASE_URL}/api/users/${testUsername}`);
    ...
    describe('Data Upload Endpoints', () => {
        it('POST /api/import should accept multipart form data', async () => {
            ...
            const response = await apiClient.upload(`${API_BASE_URL}/api/import`, formData);
    ...
    describe('Error Handling Contracts', () => {
        it('should return 404 for non-existent test run', async () => {
            await expect(async () => {
                await apiClient.get(`${API_BASE_URL}/api/test-runs/999999`);
            }).rejects.toThrow();
            try {
                await apiClient.get(`${API_BASE_URL}/api/test-runs/999999`);
            }
        ...
        it('should return 400 for invalid filter parameters', async () => {
            await expect(async () => {
                await apiClient.get(`${API_BASE_URL}/api/test-runs/?limit=-1`);
            }).rejects.toThrow();
            try {
                await apiClient.get(`${API_BASE_URL}/api/test-runs/?limit=-1`);
            }
    ...
    describe('Data Consistency Contracts', () => {
        it('filter options should reflect actual test run data', async () => {
            const testRuns = await apiClient.get(`${API_BASE_URL}/api/test-runs/`);
            const filters = await apiClient.get(`${API_BASE_URL}/api/filters/`);
            if (testRuns.length > 0) {
        ...
        it('filtered results should match filter criteria', async () => {
            ...
            if (filters.hostnames.length > 0) {
                ...
                const filteredRuns = await apiClient.get(`${API_BASE_URL}/api/test-runs/?${params.toString()}`);
        ...
        it('performance metrics should be reasonable', async () => {
            const testRuns = await apiClient.get(`${API_BASE_URL}/api/test-runs/?limit=10`);
            if (testRuns.length > 0) {
