...
describe('Component Test: Error Handling Utilities', () => {
  ...
  it('should create structured error objects', () => {
    ...
    const error = mockErrorHandling.createError(
      'network',
      'Connection failed',
      { statusCode: 500 },
      { url: '/api/test-runs', method: 'GET' }
    )
  ...
  it('should handle API errors with proper context', () => {
    ...
    const mockHandledError: AppError = {
      category: 'not_found',
  ...
  it('should create user-friendly error messages', () => {
    ...
    mockErrorHandling.createUserFriendlyMessage.mockImplementation((category: ErrorCategory) => {
      return errorMappings[category] || errorMappings.unknown
    })

    Object.entries(errorMappings).forEach(([category, expectedMessage]) => {
      const result = mockErrorHandling.createUserFriendlyMessage(category as ErrorCategory)
  ...
  it('should handle cascading error scenarios', () => {
    ...
    const mockCascadingError: AppError = {
      category: 'server',
