...
export const authStorage = {
    set: (data) => storage.set('auth', data, { ttl: 24 * 60 * 60 * 1000 }), // 24 hours
...
export const cacheStorage = {
    set: (key, data, ttlMs = 5 * 60 * 1000) => // 5 minutes default
     storage.set(`cache:${key}`, data, { ttl: ttlMs }),
    get: (key) => storage.get(`cache:${key}`),
    has: (key) => storage.has(`cache:${key}`),
