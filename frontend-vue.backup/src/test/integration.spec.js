...
describe('Integration Tests - Full User Scenarios', () => {
    ...
    describe('User Authentication Workflow', () => {
        it('should handle complete login flow', async () => {
            ...
            expect(mockApiClient.post).toHaveBeenCalledWith(expect.stringContaining('/login'), expect.objectContaining({
                username: 'test-user',
    ...
    describe('Dashboard Data Loading Workflow', () => {
        it('should load dashboard data on mount', async () => {
            ...
            expect(mockApiClient.get).toHaveBeenCalledWith(expect.stringContaining('/api/test-runs'));
            expect(mockApiClient.get).toHaveBeenCalledWith(expect.stringContaining('/api/filters'));
        });
        it('should display loading state during data fetch', async () => {
            // Mock slow API response
            mockApiClient.get.mockImplementation(() => new Promise(resolve => setTimeout(() => resolve(mockTestRuns), 100)));
            const wrapper = mount(Dashboard, {
                global: {
                    plugins: [router, pinia],
                    stubs: {
    ...
    describe('Filter Logic and Data Consistency', () => {
        ...
        it('should apply filters with AND logic correctly', async () => {
            ...
            const result = filteredData.value || mockTestRuns.filter(tr => tr.hostname === 'server-01' &&
                tr.drive_type === 'NVMe' &&
                tr.block_size === '4K' &&
