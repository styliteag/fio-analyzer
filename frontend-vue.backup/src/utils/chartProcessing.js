...
export function processHeatmapData(testRuns, metric = 'iops', options = {}) {
...
export function processScatterData(testRuns, xMetric = 'avg_latency', yMetric = 'iops') {
    if (!testRuns || testRuns.length === 0)
        return [];
...
export function processLineChartData(testRuns, xAxis = 'block_size', yAxis = 'iops', groupBy) {
    if (!testRuns || testRuns.length === 0)
        return [];
    if (groupBy) {
        ...
        testRuns.forEach(run => {
            const groupKey = run[groupBy] || 'unknown';
...
export function calculateRelativeColorScale(testRuns, metric = 'iops') {
    if (!testRuns || testRuns.length === 0) {
...
// Group data by host
export function groupByHost(testRuns) {
    const grouped = {};
    testRuns.forEach(run => {
...
// Aggregate metrics
export function aggregateMetrics(testRuns, metrics = ['iops', 'bandwidth', 'avg_latency']) {
    if (!testRuns || testRuns.length === 0)
        return {};
    const result = {};
    metrics.forEach(metric => {
        const values = testRuns.map(run => run[metric]).filter(v => v !== undefined && v !== null && v > 0);
        if (values.length === 0) {
...
export function filterDataBySelection(testRuns, filters) {
    ...
    return testRuns.filter(run => {
        ...
        for (const [category, values] of Object.entries(filters)) {
            ...
            const runValue = run[category];
            if (runValue === undefined || runValue === null)
                return false;
            // Handle different category types
            switch (category) {
                case 'hostnames':
                    if (!valueArray.includes(run.hostname))
                        return false;
                    break;
                case 'drive_types':
                    if (!valueArray.includes(run.drive_type))
                        return false;
                    break;
                case 'drive_models':
                    if (!valueArray.includes(run.drive_model))
                        return false;
                    break;
                case 'protocols':
                    if (!valueArray.includes(run.protocol))
                        return false;
                    break;
                case 'block_sizes':
                    if (!valueArray.includes(run.block_size))
                        return false;
                    break;
                case 'patterns':
                    if (!valueArray.includes(run.read_write_pattern))
                        return false;
                    break;
                case 'queue_depths':
                    if (!valueArray.includes(run.queue_depth))
                        return false;
                    break;
                case 'num_jobs':
                    if (!valueArray.includes(run.num_jobs))
                        return false;
                    break;
                case 'syncs':
                    if (!valueArray.includes(run.sync))
                        return false;
                    break;
                case 'directs':
                    if (!valueArray.includes(run.direct))
                        return false;
                    break;
                case 'test_sizes':
                    if (!valueArray.includes(run.test_size))
                        return false;
                    break;
                case 'durations':
                    if (!valueArray.includes(run.duration))
                        return false;
                    break;
            }
        }
...
// Helper functions
function createChartPoint(run, xAxis, yAxis) {
    let x;
    let y;
    // Get X value
    switch (xAxis) {
...
function getPerformanceZone(iops, latency) {
    // Simple performance zone classification
    if (latency < 1 && iops > 50000)
        return 'high_performance';
    if (latency < 5 && iops > 10000)
        return 'balanced';
    if (latency > 10)
        return 'high_latency';
...
function createHeatmapTooltip(run, avgValue, metric, count) {
...
export function stratifiedSample(data, sampleSize) {
...
export function createDebouncedChartProcessor(processingFn, delay = 150) {
    let timeoutId = null;
    return (...args) => {
        if (timeoutId) {
...
export function virtualizeChartData(data, viewportStart, viewportSize, bufferSize = 100) {
...
export function getCachedColor(value, scale) {
